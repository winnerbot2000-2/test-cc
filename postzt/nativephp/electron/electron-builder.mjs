...
export default {
    ...
    beforePack: async (context) => {
        ...
        // Run synchronously: the php.js script extracts the PHP binary and
        // chmods it executable; electron-builder must not start packaging the
        // extraResources until that (and the chmod) has completed, otherwise
        // the bundled `php` loses its executable bit (spawn EACCES).
        execSync(`node php.js --${targetOs} --${arch}`, { stdio: 'inherit' });

        // php.js's chmod runs in an async callback that newer Node versions
        // can drop on the floor, so force the executable bit synchronously
        // before electron-builder copies the binary into the bundle.
        const phpBinary = join(process.env.NATIVEPHP_BUILD_PATH, 'php', 'php');
        execSync(`chmod +x "${phpBinary}"`, { stdio: 'inherit' });
    },
    ...
    mac: {
        ...
        // Ad-hoc sign the app for local use. The PHP binary is signable once
        // the php.js extraction runs to completion (see php.js). A real Apple
        // certificate is required for notarized distribution.
        identity: '-',
