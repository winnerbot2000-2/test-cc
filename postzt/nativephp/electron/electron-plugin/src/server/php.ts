...
async function getPhpPort() {
    // TryPost: OAuth redirect URIs must be registered ahead of time with each
    // social platform, so the PHP server needs a *stable* loopback port rather
    // than a random one. Allow NATIVEPHP_PHP_PORT to pin it (default 8099) and
    // fall back to the original dynamic behaviour if that port is taken.
    const pinnedPort = parseInt(process.env.NATIVEPHP_PHP_PORT || '8099', 10);

    if (!Number.isNaN(pinnedPort) && pinnedPort > 0 && pinnedPort < 65536) {
        if (await canBindToPort(pinnedPort)) {
            return pinnedPort;
        }

        console.warn(
            `Pinned PHP port ${pinnedPort} is in use — falling back to a dynamic port. ` +
                'OAuth redirects registered against the pinned port will not work.',
        );
    }
